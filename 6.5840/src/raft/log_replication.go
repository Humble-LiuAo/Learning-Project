	...
	if args.PrevLogIndex+1 < rf.log.FirstLogIndex || args.PrevLogIndex > rf.log.LastLogIndex {
		...
		for i, entry := range args.Entries {
			...
			if index > rf.log.LastLogIndex {
				...
				// 此时index在args.prevlogindex 和 rf.log.lastlogindex 之间
				// 采用覆盖写的方式
				ok = false
		...
		// args.prevlogindex在 rf.log中 但term不匹配
		prevIndex := args.PrevLogIndex
		// 直接跳到log上一个term的 last log index
